...
public class FormularioAgregarCliente extends JFrame {
    ...
    public FormularioAgregarCliente(Refrescable pantallaPrincipal, int usuarioId) {
        ...
        // Configuramos ESTE frame (no creamos otro JFrame)
        ...
        // Fondo gradiente (si luego quieres usarlo, está disponible)
        GradientPanel root = new GradientPanel(BG_TOP, BG_BOT);
        root.setLayout(new GridBagLayout());

        // Esquinas redondeadas sobre este frame
        ...
        // Clip redondeado para tu mainPanel (usa el RoundedClipUI del Login)
        JLayer<JComponent> roundedLayer = new JLayer<>(mainPanel, new Login.RoundedClipUI(10));
        setContentPane(roundedLayer);

        // ===== Tipografía básica =====
        Font fTextLocal = new Font("Segoe UI", Font.PLAIN, 16);
        if (txtNombre != null) txtNombre.setFont(fTextLocal);
        if (txtTelefono != null) txtTelefono.setFont(fTextLocal);
        if (txtCurp != null) txtCurp.setFont(fTextLocal);
        if (txtRFC != null) txtRFC.setFont(fTextLocal);
        if (txtCorreo != null) txtCorreo.setFont(fTextLocal);
        if (txtNSS != null) txtNSS.setFont(fTextLocal);
        if (btnAgregar != null) btnAgregar.setFont(fTextLocal);
        if (btnCancelar != null) btnCancelar.setFont(fTextLocal);

        // ===== Campos con borde reactivo =====
        ...
        if (btnCancelar != null) {
            styleExitButton(btnCancelar); // rojo
            btnCancelar.addActionListener(e -> {
                dispose(); // ahora sí cierra esta ventana
    ...
    // Clip redondeado sin tocar la jerarquía (no se usa, pero disponible)
    static class RoundedClipUI extends LayerUI<JComponent> {
        private final int arc;

        RoundedClipUI(int arc) {
            this.arc = arc;
        }
    ...
    private void styleTextField(JTextField tf) {
        if (tf == null) return;  // evita NPE si el campo no está mapeado
        tf.setOpaque(true);
    ...
    static class CheckIcon implements Icon {
        ...
        private static final Color BOX_BORDER = new Color(0x535353);
        private static final Color BOX_BG_HOVER = new Color(0xF3F4F6);
        private static final Color BOX_BG_SELECTED = new Color(0x16A34A);
        private static final Color BOX_BG_SEL_HOV = new Color(0x22C55E);
        private static final Color CHECK_COLOR = Color.WHITE;
        private static final Color FOCUS_RING = new Color(0x059669);
